...
def plot_mle_normal(params, data, censored, output_file):
    """Построение графиков для MLE нормального распределения"""
    ...
    x = np.linspace(data.min() - std, data.max() + std, 1000)
    pdf = stats.norm.pdf(x, mean, std)
    ax.plot(x, pdf, 'r-', linewidth=2.5, label=f'N(μ={mean:.2f}, σ={std:.2f})')
    ...
    info_text = f'μ = {mean:.4f} ± {params.get("std_error_1", 0):.4f}\nσ = {std:.4f} ± {params.get("std_error_2", 0):.4f}\nn = {len(complete_data)}'
...
def plot_mls_normal(params, data, censored, output_file):
    """Построение графиков для MLS нормального распределения (с цензурой)"""
    mean = params['parameter_1']
    std = params['parameter_2']
    ...
    # Теоретическое распределение
    x = np.linspace(data.min() - std, data.max() + 2*std, 1000)
    pdf = stats.norm.pdf(x, mean, std)
    ax.plot(x, pdf, 'r-', linewidth=2.5, label=f'N(μ={mean:.2f}, σ={std:.2f})')
